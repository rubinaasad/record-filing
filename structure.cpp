// struct filing.cpp : Defines the entry point for the console application.
...
char sFileName[32]="D:\\result.txt";
void write_data(Student st[],int Max)
{
...
void read_data(Student st[],int Max)
{
	...
	while(f.eof()==false)
	{
		if(f>>st[cur_size].nRollNo>>st[cur_size].name>>st[cur_size].dMarks>>st[cur_size].cGrade)
			cur_size++;
	}
	f.close();
}
...
void search_student(Student st[],int n)
{
	bool found=false;
	for(int i=0;i<cur_size;i++)
	{
...
void modify_student(Student st[],int n)
{
	bool found=false;
	for(int i=0;i<cur_size;i++)
	{
...
void delete_student(Student st[],int Max,int &cur_size,int n)
{	
	bool flag=false;
	for (int i=0;i<cur_size;i++) 
	{
